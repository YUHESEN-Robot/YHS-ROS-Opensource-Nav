...
namespace timoo_pointcloud
{
  ...
  Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh, std::string const & node_name):
    data_(new timoo_rawdata::RawData( node, private_nh)), first_rcfg_call(true),
    diagnostics_(node, private_nh, node_name)
  {
    // Get startup parameters
    private_nh.param<std::string>("fixed_frame", config_.fixed_frame, "timoo");
    private_nh.param<std::string>("target_Frame", config_.target_frame, "timoo");
    ...
    boost::optional<timoo_pointcloud::Calibration> calibration = data_->setup(private_nh);
    if(calibration)
    {
    ...
    output_ =
      node.advertise<sensor_msgs::PointCloud2>("timoo_points", 10);

    srv_ = boost::make_shared <dynamic_reconfigure::Server<timoo_pointcloud::
    ...
    timoo_scan_ =
      node.subscribe("timoo_packets", 10,
                     &Convert::processScan, (Convert *) this,
                     ros::TransportHints().tcpNoDelay(true));
    timoo_difop_ =
      node.subscribe("timoo_status", 10,
                     &Convert::processDifop, (Convert *) this,
                     ros::TransportHints().tcpNoDelay(true));
    ...
    diag_topic_.reset(new TopicDiagnostic("timoo_points", diagnostics_,
