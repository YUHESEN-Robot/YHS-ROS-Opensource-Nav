...
namespace yocs_velocity_smoother
{
  ...
  **********************/
  ...
  VelocitySmoother::VelocitySmoother(const std::string &name)
      : name(name), quiet(false), shutdown_req(false), input_active(false), pr_next(0), dynamic_reconfigure_server(NULL){};
  ...
  void VelocitySmoother::FwSteeringCtrlCmdCB(const yhs_msgs::FwSteeringCtrlCmd::ConstPtr &msg)
  {
    ctrl_cmd_last_cb_time = ros::Time::now();
    input_active = false;
  }
  ...
  void VelocitySmoother::spin()
  {
    ...
    while (!shutdown_req && ros::ok())
    {
      ...
      // 没有接收到速度，发布0
      if ((ros::Time::now() - last_cb_time).toSec() > 0.2 && (ros::Time::now() - ctrl_cmd_last_cb_time).toSec() > 0.2)
      {
           ...
           v_different_from_feedback || w_different_from_feedback))
      {
        ...
        if (!quiet)
        {
          ...
          ROS_WARN_STREAM("Velocity Smoother : using robot velocity feedback " << std::string(robot_feedback == ODOMETRY ? "odometry" : "end commands") << " instead of last command: " << (ros::Time::now() - last_cb_time).toSec() << ", " << current_vel.linear.x - last_cmd_vel.linear.x << ", " << current_vel.angular.z - last_cmd_vel.angular.z << ", [" << name << "]");
        }
          ...
          (target_vel.angular.z != last_cmd_vel.angular.z))
      {
        ...
        if (target_vel.linear.x <= 0 /*&& (chassis_type == "DT" || chassis_type == "FW")*/)
        {
  ...
  bool VelocitySmoother::init(ros::NodeHandle &nh)
  {
    ...
    ros::NodeHandle ph_1("~");
    ph_1.param("/common/chassis_type", chassis_type, std::string("FW"));

    ROS_INFO("/common/chassis_type:%s", chassis_type.c_str());
    ...
    ros::NodeHandle ph_2;
    if (chassis_type == "FW")
      fw_ctrl_cmd_sub = ph_2.subscribe<yhs_msgs::FwSteeringCtrlCmd>("fw_steering_ctrl_cmd", 10, &VelocitySmoother::FwSteeringCtrlCmdCB, this);
